...
export default function Chatheader({selectedUser,setselectedUser,onlineUsers}){
    return(
        <div className="flex h-17 w-full border-b-grey-500 flex px-5 items-center gap-2">
            <button className="text-white cursor-pointer" onClick={()=>{setselectedUser(null)}}><ChevronLeft/></button>
            <img className="h-12 w-12 border rounded-full border-grey-400" src={selectedUser.profilepic || "/profile.png"} alt={selectedUser.fullname}/>
